...
output_root="${repo_root}/exps/decoder_ablation/frozen_pretrained_encoder_xdocked_20260728"
...
idle_checks=0
echo "Waiting for physical GPUs 2 and 3 before frozen-encoder training."
while true; do
...
echo "GPUs are idle; launching frozen-pretrained-encoder decoder training."
(
    cd "${repo_root}"
    exec env \
        ...
        "${python_bin}" -u -m funcbind.train_frozen_decoder \
        variant=inr \
