...
sample_dir="${experiment_dir}/samples_epoch500_holo_density_target69"
density_dir="${voxbind_root}/dataset/data/xray_crops_aligned_v5"

if [[ ! -f "${experiment_dir}/checkpoint_epoch_0500.pth.tar" ]]; then
    echo "epoch-500 milestone checkpoint not found" >&2
    exit 1
fi
if [[ -f "${sample_dir}/target_69/samples.sdf" ]]; then
    echo "[$(date --iso-8601=seconds)] epoch-500 target-69 samples already exist; skipping"
    exit 0
fi
...
echo "[$(date --iso-8601=seconds)] sampling epoch-500 60M model on test target 69 with real holo density"
cd "${voxbind_root}"
"${runtime}/bin/python" sample.py \
    ...
    save_dir="${sample_dir}" \
    out_dir=epoch500_holo_density_target69 \
    seed=1269 \
    num_workers=0 \
...
molecule_count="$(rg -c '^\$\$\$\$$' "${sample_path}" || true)"
echo "[$(date --iso-8601=seconds)] epoch-500 target-69 sampling complete: ${molecule_count} molecules"
